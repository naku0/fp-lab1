...
help :: Int -> Int
help = sum . map ((^ (5 :: Integer)) . digitToInt) . show

-- 1
recursion :: Int
recursion = innerFunc (6 * (9 ^ (5 :: Integer)))
  where
    innerFunc 1 = 0
    innerFunc limit
...
linearRecursion :: Int
linearRecursion = innerFunc (6 * (9 ^ (5 :: Integer))) 0
  where
...
-- 5
listiik :: Int
listiik = sum [x | x <- takeWhile (<= 6 * (9 ^ (5 :: Integer))) [2 ..], x == help x]
